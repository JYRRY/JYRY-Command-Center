...
1. If `M<N> <tier>`: call `mcp__jyry__bulk_prepare({milestone_id, tier})` to get the batch of tasks needing prepare.
   If `M<N> all`: call `mcp__jyry__bulk_prepare({milestone_id})` with no tier filter so MCP returns all non-small tasks needing prepare.
   If `T<id>`: call `mcp__jyry__get_task_context(task_id)` for the single task.
...
5. Call `mcp__jyry__enrich_task(...)` to:
