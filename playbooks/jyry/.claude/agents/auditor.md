...
allowed-tools:
  ...
  - mcp__jyry__get_task_summary
  - mcp__jyry__request_audit
  - mcp__jyry__submit_audit
...
Call `mcp__jyry__request_audit(task_id, cross_model?)`. It returns:
...
Call `mcp__jyry__submit_audit(task_id, results, auditor_id='auditor', auditor_summary)`. The `results` array contains all 12 items with filled-in status + detail.
