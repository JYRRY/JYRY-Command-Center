...
This checklist defines the future platform cutover from JYRY-first compatibility to a consumer-profile model. It is planning only. Do not execute these changes implicitly.

## Preconditions

- The public boundary doc is accepted.
- The JYRY coupling audit is accepted.
- A compatibility plan exists for the live JYRY repo and its `.mcp.json` wiring.
...
- Keep `JYRY Command Center` / `jyry-command-center` as the primary public identity.
- Decide whether the `jyry` CLI alias remains as a secondary compatibility entrypoint.
- Update operator-facing docs to lead with the platform name and list JYRY as a supported profile.

## Config and Resolution

- Replace implicit sibling `../jyry` discovery with explicit project-profile resolution.
- Move `JYRY_PROJECT_ROOT` to a documented compatibility alias only.
...
- Decide whether `jyry-tracker.json` remains a compatibility filename or is migrated.
- Provide a generic parser command layer only if it remains explicit about profile and source.
- Add migration guidance for existing JYRY trackers if parser metadata becomes mandatory platform-wide.
...
- Introduce generic package and binary names without removing the existing JYRY entrypoints immediately.
...
- Only remove JYRY-first defaults after the compatibility window is complete.

## Validation Gates

- Local builds pass for both the app and MCP server.
- Live JYRY compatibility remains intact during the transition window.
...
- changing the current JYRY `.mcp.json` wiring today
- changing runtime or MCP tracker write behavior today
- forcing the live JYRY repo onto a new tracker filename today
