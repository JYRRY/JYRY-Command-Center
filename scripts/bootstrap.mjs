...
function printHelp() {
  console.log(
    'JYRY Command Center bootstrap (jyry-command-center)\n' +
    '\n' +
    'Usage:\n' +
    ...
    '\n' +
    'JYRY compatibility:\n' +
    '  npm run bootstrap -- --project ../jyry --profile jyry\n' +
    '\n' +
    'Options:\n' +
    '  --project <path>           Project root that owns the tracker and docs. Required.\n' +
    '  --profile <id>            Consumer profile. Defaults to generic unless JYRY is inferred.\n' +
    ...
    '  JYRY_PROJECT_ROOT is only written when the jyry profile is selected for compatibility.\n'
  )
}
...
function buildEnvLines(existing, profile, generatedKeys) {
  ...
  return [
    ...
    profile.id === 'jyry'
      ? '# JYRY compatibility bootstrap. COMMAND_CENTER_* remains the primary configuration surface.'
      : '# Public-first bootstrap for external JYRY Command Center projects.',
    profile.id === 'jyry'
      ? '# JYRY_PROJECT_ROOT is included below for legacy compatibility only.'
...
function inferProfileId({ profile, project, tracker }) {
  ...
  if (tracker && basename(tracker) === 'jyry-tracker.json') return 'jyry'
  if (existsSync(join(resolve(project), 'jyry-tracker.json'))) return 'jyry'
  if (resolve(project) === SIBLING_JYRY_ROOT) return 'jyry'
  return 'generic'
}
...
if (profileId !== 'generic' && profileId !== 'jyry') {
  console.error(`Unknown consumer profile "${profileId}". Expected "generic" or "jyry".`)
  process.exit(1)
}
...
async function main() {
  ...
  if (args.checklist || profile.id === 'jyry') {
  ...
  if (args.manifesto || profile.id === 'jyry' || existsSync(join(resolvedProjectRoot, manifestoPath))) {
  ...
  if (profile.id === 'jyry') {
    existing.set('JYRY_PROJECT_ROOT', resolvedProjectRoot)
    if (!generatedKeys.includes('JYRY_PROJECT_ROOT')) {
      generatedKeys.push('JYRY_PROJECT_ROOT')
    }
  } else {
    existing.delete('JYRY_PROJECT_ROOT')
  }
  ...
  console.log(
    profile.id === 'jyry'
      ? 'Compatibility: JYRY_PROJECT_ROOT was written for existing JYRY installs.'
      : 'Compatibility: no JYRY-specific env keys were written.'
  )
  console.log('')
  ...
  console.log('3. Dry-run `npm run tracker:parse:jyry-tasks:dry-run` before any compatibility write.')
  console.log('4. Write only after the dry-run looks correct with `npm run tracker:parse:jyry-tasks`.')
}
