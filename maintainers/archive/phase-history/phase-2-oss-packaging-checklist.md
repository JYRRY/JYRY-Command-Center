...
- define the target root package name as `jyry-command-center`
- define the target MCP package name as `jyry-command-center-mcp`
...
- keywords describe the platform, not only JYRY
- package descriptions identify JYRY as a compatibility profile, not the product identity

## Release Structure

- root app package is buildable from a clean install
- MCP package is buildable from a clean install
- the public repo does not require the private or live JYRY repo at publish time
- publish artifacts do not assume sibling `../jyry`

## Public Docs

- README leads with the platform identity
- setup docs explain explicit profile selection
- JYRY docs are labeled as compatibility guidance
- release docs explain the compatibility window and legacy aliases

## Bootstrap and Config

- bootstrap supports a public `generic` profile path
- bootstrap can still emit JYRY-compatible config when explicitly requested
- `.env.example` shows public defaults first
- hidden JYRY assumptions are removed from fresh-install docs
...
- document how to opt into the JYRY compatibility profile

## Packaging Safety

- no package publish step writes to a live JYRY tracker
- no release docs instruct operators to mutate live JYRY wiring as part of install
- script-side safety docs remain accurate after the rename

## Compatibility Window

- keep the `jyry` CLI alias during the transition window
- keep `JYRY_PROJECT_ROOT` documented as a compatibility alias during the transition window
- keep `jyry-tracker.json` compatibility documented during the transition window
- document the eventual removal conditions before removing any alias

## Release Readiness Gate

- clean install works without a sibling JYRY checkout
- app build passes
- MCP build passes
