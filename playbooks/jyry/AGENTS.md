...
|---|---|---|
| `next` (with optional tier or milestone args) | Call `mcp__jyry__get_next_actionable_tasks`. Format the result as a table grouped by tier. READ-ONLY — do not execute any task. | `.claude/commands/next.md` |
...
## MCP tools available (via JYRY MCP server)
...
Use `./jyry` only as a shell fallback when MCP is unavailable. Never edit `jyry-tracker.json` directly.
