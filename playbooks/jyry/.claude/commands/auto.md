...
Per wave, tasks of the same tier and with no file collisions run in parallel via Agent subagents. Between waves, sequential — the next wave must wait for the previous wave's auto-approvals to fire.
