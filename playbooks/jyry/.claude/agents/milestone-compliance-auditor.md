...
allowed-tools:
  ...
  - mcp__jyry__get_checklist_status
...
- `mcp__jyry__get_checklist_status()` → all categories → filter where `category.linked_milestone === milestone_id`
...
| "migration applies cleanly" | `cd jyry && npx prisma migrate status` |
